...
class RegistryTests: XCTestCase {
    ...
    func testTransmitMethodReturns() {
        ...
        let request = Request(proc: Task.Form(.Validate(.AddSession)), body: ExerciseSessionViewModel(dateFormatter: formatter, name: "name", date: Date(), duration: Date()))
        registry.register(inputHandler: controller)
    ...
    func testTransmitMethodReturnsError() {
        ...
        let registry = ResponderRegistry()
        let formatter = DateFormatter()
        let request = Request(proc: Task.Exercise(.add), body: ExerciseSessionViewModel(dateFormatter: formatter, name: "name", date: Date(), duration: Date()))
            registry.register(inputHandler: controller)
        let promise = registry.tx(request: request)
